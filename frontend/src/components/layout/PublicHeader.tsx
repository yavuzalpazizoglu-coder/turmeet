  ...
  return (
    ...
    >
      <div className="flex h-28 w-full items-center justify-between pl-2 pr-4 sm:pl-3 sm:pr-6">
        ...
        {/* Desktop nav — gövde metinleriyle eşit boyut (14px) */}
        <nav className="hidden items-center gap-6 md:flex">
          {NAV.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className={`text-base font-bold leading-none transition-colors ${transparent ? "text-white/90 hover:text-white" : "text-ink hover:text-brand"}`}
            >
              {item.label}
            </Link>
          ))}
